...
### GET-ting our Initial Data
Now that everything is setup and we can actually run Python code within the notebook, it's time to pick an API and make a request to actually get the data. If you aren't sure where to start looking for an API, [PublicAPI](https://publicapi.dev) has a great site of publicly available datasets. Many of which are accesible both programmatically or through GUIs. 

Before we jump into the code, two quick notes about HTTP and GET:
...
Besides my interest in space, the other perk of using NASA's API is that it's free and provides a reasonable amount (1,000) of hourly calls. That being said, the documentation isn't perfect, but it's definitely easy enough to query. Below, I'll share the first few cells of my notebook.
...
start_date, end_date = "2024-12-01", "2024-12-08"
